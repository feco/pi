...
5. **Review once, in parallel.** After all slices are implemented and verified, dispatch in one fresh-context workflow:
   ...
   Give reviewers the user goal, approved scope and decisions, repository location, exact change boundary (including relevant untracked files), resolved revisions, current index result, relevant scoped diff and verification results. Supply the diff inline or as an authorized readable artifact for headless reviewers. Follow `~/.pi/agent/GUARDRAILS.md` for evidence attribution.
